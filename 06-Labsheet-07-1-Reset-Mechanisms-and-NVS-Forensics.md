...
| **2. Menuconfig Flag**           | `CONFIG_EXAMPLE_RESET_PROVISIONED=y` |ทุกครั้งที่เปิดเครื่อง Credentials ถูกล้าง LED อยู่สถานะ Disconnected ไฟกระพริบ 200ms|[STATUS]: Device is NOT provisioned (NVS is empty) |
...
```
เพราะ GPIO 0 เป็น Strapping Pin ถ้าถูกดึงเป็น LOW ตอน Reset เปิดเครื่อง ESP32 จะเข้า ROM Bootloader
```
...
```
เพราะ idf.py erase-flash ล้าง Flash ทั้งชิป ส่วน nvs_flash_erase() ล้างเฉพาะ พื้นที่ NVS
```
...
```
ป้องกันการกดโดยไม่ตั้งใจ เพราะการ Reset จะลบข้อมูลสำคัญออกไป
```
4. หากอุปกรณ์ IoT ถูกติดตั้งอยู่บนเสาสูงหรือฝังอยู่ในผนัง วิธีการ Reset ทางกายภาพรูปแบบใดเหมาะสมที่สุด?
```
เหมาะกับ Software Remote Reset หรือ CLI Reset เพราะไม่ต้องเข้าถึงปุ่มบนตัวอุปกรณ์โดยตรง
```
