...
<img width="537" height="807" alt="image" src="https://github.com/user-attachments/assets/4d9193ca-3d4a-447a-a1cf-b307f42ea57c" />
...
| :--- | :--- |
| **1. ชื่อ SoftAP SSID ของ ESP32** | `PROV_`PROV_DACE4C |
| **2. รหัส PoP (Proof of Possession)** |abcd1234 |
| **3. ข้อความใน QR Code Payload (JSON)** | {"ver":"v1","name":"PROV_DACE4C","pop":"abcd1234","transport":"softap"}|
| **4. พฤติกรรมไฟ LED 3 (GPIO 5) ช่วงรอ vs ช่วงส่งข้อมูล** | ช่วงรอ: LED กระพริบเพื่อแสดงสถานะรอการ Provisioning <br/>ช่วงส่ง: LED เปลี่ยนสถานะเพื่อแสดงว่ากำลังดำเนินการ Provisioning|
| **5. IP Address ที่ ESP32 ได้รับจาก Router** | 172.20.10.2 |
| **6. เวลาที่ใช้ตั้งแต่เริ่มจนจบกระบวนการ (วินาที)** | ประมาณ 15–20 วินาที |
...
```
 ใช้โปรโตคอล HTTP และใช้ IP Address 192.168.4.1
```
...
```
ป้อนรหัสผ่าน Wi-Fi ผิดจะเกิด WIFI_PROV_CRED_FAIL ESP32 จะแจ้งว่า Provisioning ล้มเหลว และสามารถให้ผู้ใช้ลองส่ง Credentials ใหม่ได้
```
3. ทำไมผู้ผลิต IoT ส่วนใหญ่จึงมองว่ากระบวนการเชื่อมต่อแบบ SoftAP มีขั้นตอนที่ยุ่งยากสำหรับผู้ใช้ทั่วไปเมื่อเทียบกับ BLE?
```
เพราะ SoftAP ต้องสลับไปเชื่อมต่อ Wi-Fi ของอุปกรณ์เอง แล้วกลับมาใช้แอปเพื่อ Provisioning ทำให้มีหลายขั้นตอน ส่วน BLE จะเชื่อมต่อกับอุปกรณ์ผ่านแอปได้โดยตรง
```
